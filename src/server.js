...
app.post('/join', (req, res) => {
  ...
  const count = db.prepare('SELECT COUNT(*) as count FROM customers WHERE restaurantId = ?').get(restaurantId);
  const position = count.count + 1;
  db.prepare('INSERT INTO customers (name, phone, restaurantId, position) VALUES (?, ?, ?, ?)').run(name, formattedPhone, restaurantId, position);
...
app.post('/table-free', (req, res) => {
  ...
  const next = db.prepare('SELECT * FROM customers WHERE restaurantId = ? ORDER BY position ASC LIMIT 1').get(restaurantId);
  if (next) {
    db.prepare('DELETE FROM customers WHERE id = ?').run(next.id);
...
app.get('/waitlist/:restaurantId', (req, res) => {
  const list = db.prepare('SELECT * FROM customers WHERE restaurantId = ? ORDER BY position ASC').all(req.params.restaurantId);
  res.json(list);
});
...
app.get('/qrcode/:restaurantId', async (req, res) => {
  ...
  const url = `https://jaywalker-refract-abstract.ngrok-free.dev/?restaurant=${restaurantId}`;
